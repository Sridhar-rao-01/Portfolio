      ...
      <div className="max-w-7xl mx-auto px-6" ref={ref}>
        ...
        >
          ...
          <h2 className="section-heading text-foreground">
            My{" "}
            <span className="text-neon-green" style={{ textShadow: "0 0 10px #39ff1466, 0 0 20px #39ff1433" }}>
              Learning Journey
            </span>
          </h2>
          <p className="mt-4 text-muted max-w-2xl">
            A structured roadmap from UI/UX design through full stack development to DevOps engineering.
          </p>
        </motion.div>

        <div className="relative">
          <div className="absolute left-8 md:left-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-accent via-accent-alt to-neon-green opacity-30" />
            ...
            {roadmap.map((phase, i) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`w-full md:w-[calc(50%-2rem)] ${isLeft ? "md:pr-12 md:text-right" : "md:pl-12"} pl-16 md:pl-0`}>
                    <div className={`p-6 rounded-xl border ${statusColor} transition-all`}>
                      <div className={`flex items-center gap-2 mb-2 ${isLeft ? "md:justify-end" : ""}`}>
                        <span className="text-xs font-[family-name:var(--font-geist-mono)] text-accent tracking-wider">
                          {phase.phase}
                        </span>
                        {phase.status === "completed" && (
                      ...
                      <ul className={`space-y-1.5 ${isLeft ? "md:text-right" : ""}`}>
                        {phase.items.map((item) => (
                          ...
                          >
                            {isLeft && <span className="hidden md:inline flex-1" />}
